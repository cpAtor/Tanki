...
namespace FusionExamples.Tanknarok
{
	...
	public class Player : NetworkBehaviour, ICanTakeDamage, IStateAuthorityChanged
	{
		...
		[FormerlySerializedAs("_playerShooting")] [SerializeField] private WeaponManager weaponManager;
		
		[HideFromInspector, Networked(OnChanged = nameof(OnStateChanged))]
		public State state { get; set; }

		[HideFromInspector, Networked]
		...
		private Vector3 _lastMoveDirection; // Store the previous direction for correct hull rotation
		...
		private void SetMeshOrientation()
		{
			...
			if(aimDirection.sqrMagnitude>0)
				_turret.forward = Vector3.Lerp(_turret.forward, aimDirection, Time.deltaTime * 100f);
		}
		...
		public void ApplyDamage(Vector3 impulse, byte damage, PlayerRef attacker)
		{
			...
			// Check if we're dead, only the server is allowed to trigger the death explosion since we don't want 
			// the death explosion to be mis-predicted which looks awful. The client will predict the damage, 
			// but only shows the damage flash
			if (damage >= life)
			{
				life = 0;
				state = State.Dead;
				
				if(GameManager.instance.playState==GameManager.PlayState.LEVEL)
					lives -= 1;
		...
		private void CheckRespawn()
		{
			...
			if (_respawnInSeconds <= 0)
			{
				...
				Transform spawn = GetLevelManager().GetPlayerSpawnPoint(playerID).transform;
		...
		{
			if(Object.InputAuthority!=Object.StateAuthority) // Another player has disconnected from shared mode and we've been given authority over their player, just kill it!
				TriggerDespawn();
		}
